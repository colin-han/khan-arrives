...
2. **生成产物**，放在 `{YYYY-MM-DD}-{slug}/` 目录下：
   ...
   - `text.txt` — 优化后的朋友圈文案
   - **同步更新预览菜单兜底列表**：把新目录名加进 `preview.html` 的 `FALLBACK_TOPICS` 数组（最新在前）。左侧菜单平时靠目录自动发现（解析本地 server 的目录索引），但兜底列表在自动发现失效时使用，每新增话题都要记得同步。
...
- **比例**（2026-07-22 更新）：**优先 3:4 竖版**，画布 `1080px × 1440px`（朋友圈单图显示效果好）；**内容需要时可用 4:3 横版**（如宽结构的架构图、多列并排排不下时），画布 `1440px × 1080px`。横版注意：① 截图/导出命令的 `--window-size` 相应改为 `1440,1080`（导出 2880×2160）；② `preview.html` 会从 image.html 的 `html,body{width:...px;height:...px}` 自动识别横竖版，画布尺寸务必写在这条 CSS 里。
- **风格**：手绘插画质感（2026-07-18 调整，原「浅色极简 / 编辑风」）。强调「个人创作感」——线条有温度、可以不完美；**避免「找张照片配个标准标题」的素材拼接 / 模板感**。暖白底（`#faf7f2`）、墨色文字、单一强调色（朱红 `#d4441c`）保留。
- **主题色按话题分**（2026-07-23 确认）：工作 / AI 开发类配图用**浅色主题**（暖白底 `#faf7f2` + 墨字 + 朱红 `#d4441c` 强调）；**写小说 / 创作类**话题用**深色主题**（暖墨底 `#1f1a14` + 暖米字 `#ece1cc` + 暖橙 `#e8843a` 强调）。
...
- **署名**（2026-07-21 更新格式）：左下角「Khan驾到」（宋体 + 强调色短横线），后面跟一句小字灰调的**思考主题标签**（21px、正文字体），当前为「怎样让人和 Agent 协作得更好？」——主题随用户当下关注的问题演进，改前询问。右下角可放英文主题词。
...
- **以用户初稿为骨架**（2026-07-21 确认）：用户给了初稿时，**保留初稿的核心判断和概念框架**（如「按漂移风险分工」这类划分标准），只做轻润色，不重写。我的贡献是把初稿里的概念**落到具体场景细节**（工具名、单号、动作），以及修笔误、调口语化——概念骨架是用户的，血肉细节我来补。
- **骆小普的英文名：Lupin**（2026-08-27 确认）：骆小普是 Loop 工作流的拟人化数字员工；英文名统一写 `Lupin`（读音 /ˈluːpɪn/，倒回「骆…普」，里面藏着 Loop，本身也是真实英文姓氏，带点笨拙温和的实习生气质）。配图右下角英文主题词、图里标签、Teams 汇报中提到他时都用这个拼写，不要写成 Looper / Loopy / Luo Xiaopu。
- **角色视角**（2026-07-21 确认）：用户身份是 **developer leader**。「我」只用于叙述者视角（我在想、和同事聊）；当文案描述**团队的开发流程**时，流程中的执行角色写「developer」，不能替换成「我」——那是给团队的愿景，不是用户自己的日常。图里的泳道/角色标签同理。
...
4. **Teams 发送成品**（2026-07-21 新增，2026-07-24 更新格式）：推送完成后，用 `agent-teams` skill 把本次生成/修改话题的成品发给我（kejing.han@essexlg.com，即发到 Teams 的 self 笔记线程 `48:notes`）。**严格发两条消息**：第一条=纯文案（`text.txt` 原文，不加任何其他文字）；第二条=纯图片 GitHub 链接（`https://github.com/colin-han/khan-arrives/blob/main/{目录}/image.png`，不加任何其他文字，如「配图：」等前缀也不要）。`file upload` 对 self 线程不支持（返回 HTTP 503），别再试附件上传。
